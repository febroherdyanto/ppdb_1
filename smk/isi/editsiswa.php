...
<?php
$t1 = mysqli_query($koneksi,"select * from bio_siswa where no_sementara='$getno'");
...
<table width="100%">
	...
	<tr>
		...
		<td width="70%">: <input type="text" name="sementara" value="<?php echo $bio['no_sementara'];?>" required readonly></td>
	</tr>
	<tr>
	...
	<?php
	$t2 = mysqli_query($koneksi,"select * from kesehatan where no_sementara='$getno'");
	...
	<tr>
		...
		<td>: <textarea name="fisik" cols="40" rows="2"><?php echo $kes['fisik'];?></textarea></td>
	</tr>
	...
	<?php
	$t4 = mysqli_query($koneksi,"select * from persyaratan where no_sementara='$getno'");
	$sy = mysqli_fetch_array($t4); ?>
	<tr>
		<td>Formulir Pendaftaran</td>
		<td>: <?php 
		if($sy['formulir'] == 'Y'){
		echo '<input type="radio" name="formulir" value="Y" checked>Y
		<input type="radio" name="formulir" value="N">N</td>';
		}else if($sy['formulir'] == 'N'){
		echo '<input type="radio" name="formulir" value="Y">Y
		<input type="radio" name="formulir" value="N" checked>N</td>';
		}  ?>
	</tr>
	<tr>
		...
		<td>: <?php 
		if($sy['fc_raport'] == 'Y'){
		echo '<input type="radio" name="fc_raport" value="Y" checked>Y
		<input type="radio" name="fc_raport" value="N">N</td>';
		}else if($sy['fc_raport'] == 'N'){
		echo '<input type="radio" name="fc_raport" value="Y">Y
		<input type="radio" name="fc_raport" value="N" checked>N</td>';
		}  ?>
	</tr>
	<tr>
		...
		<td>: <?php 
		if($sy['fc_akte'] == 'Y'){
		echo '<input type="radio" name="fc_akte" value="Y" checked>Y
		<input type="radio" name="fc_akte" value="N">N</td>';
		}else if($sy['fc_akte'] == 'N'){
		echo '<input type="radio" name="fc_akte" value="Y">Y
		<input type="radio" name="fc_akte" value="N" checked>N</td>';
		}  ?>
	</tr>
	<tr>
		<td>Jumlah Pas Foto 3x4 (3 Lembar)</td>
		<td>: <input type="number" name="jml_foto" value="<?php echo $sy['jml_foto'];?>" required>
	</tr>
	<tr>
		<td>Surat Keterangan Rangking Smt 5</td>
		<td>: <?php 
		if($sy['surat_rangking'] == 'Y'){
		echo '<input type="radio" name="rangking" value="Y" checked>Y
		<input type="radio" name="rangking" value="N">N</td>';
		}else if($sy['surat_rangking'] == 'N'){
		echo '<input type="radio" name="rangking" value="Y">Y
		<input type="radio" name="rangking" value="N" checked>N</td>';
		}  ?>
	</tr>
	<tr align="center">
...
$query1 = mysqli_query($koneksi,"update bio_siswa set nama_siswa='$nama_siswa', tempat_lahir='$tempat_lahir',
	...
	where no_sementara='$getno'") or die("GAGAL UPDATE BIODATA : ".mysqli_error($koneksi));
...
$query2 = mysqli_query($koneksi,"update kesehatan set tinggi_badan='$tinggi',berat_badan='$berat',
	buta_warna='$buta_warna',fisik='$fisik' where no_sementara='$getno'")
...
$query4 = mysqli_query($koneksi,"update persyaratan set formulir='$formulir',fc_raport='$fc_raport',
	fc_akte='$fc_akte',jml_foto='$jml_foto',surat_rangking='$rangking' where
	no_sementara='$getno'") or die("GAGAL INPUT DATA SYARAT,karena ".mysqli_error($koneksi));
